...
# Models = np.array([load_model(os.path.join(Path, 'best_val_loss.hdf5')) for Path in ['./Model_1', './Model_2', './Model_3', './Model_4', './Model_5']])
# define Models with glob and best_val_loss.hdf5
# Models = np.array([load_model(Model) for Model in glob.glob('./Model_*/best_val_loss.hdf5')])
Model=load_model('./model/best_val_loss.hdf5')
...
def Predict(Characters, Evaluate = False):
    ...
    for Characters in Characters:
        ...
        for Character in Characters:
            ...
            # define y predict for singel model
            y = np.array([np.argmax(Model.predict(x))])
